import { AnyObject, StorageResponse } from '@infra/types';

export function saveObjectOnStorage(
  data: AnyObject,
  keyName: string | null = null
): StorageResponse {
  return saveObjectAsJsonOnStorage(data, keyName);
}
export function saveObjectAsJsonOnStorage(
  data: AnyObject,
  keyName: string | null = null
): StorageResponse {
  const storageResponse: StorageResponse = saveStringOnStorage(
    JSON.stringify(data as AnyObject),
    keyName
  );
  return {
    ...storageResponse,
...
export function saveStringOnStorage(
  data: string,
  keyName: string | null = null
): StorageResponse {
